...
<head runat="server">
    <title>Bulk Media Update Tool</title>
    <style>
        body {
    ...
    public static class StreamExtension
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            {
                Response.Redirect("login.aspx?returnUrl=bulkmediaimport.aspx");
            }
        }
        ...
        protected void btnPublish_Click(object sender, EventArgs e)
        {
                ...
                if (!string.IsNullOrEmpty(txtMediaParentPath.Text))
                {
                    ...
                    if (db != null)
                    {
                        ...
                        if (parentMediaFolderItem != null && parentMediaFolderItem.HasChildren)
                        {
                            try
                            {
                                ZipStorer zip = ZipStorer.Open(uploadedFile.PostedFile.InputStream, FileAccess.ReadWrite);
                                ...
                                foreach (ZipStorer.ZipFileEntry entry in dir)
                                {
                                    try
                                    {
                                        var _filename = spath + "\\bulkmedia\\" + entry.FilenameInZip;
                                        bool success = zip.ExtractFile(entry, _filename);

                                        if (success)
                                        {
                                            var nameToMatchMedia = System.IO.Path.GetFileNameWithoutExtension(_filename);

                                            var imageItem = parentMediaFolderItem.Children.Where(i => i.Name == nameToMatchMedia).FirstOrDefault();
                                            if (imageItem != null)
                                            {
                                                Sitecore.Resources.Media.Media imageMedia = Sitecore.Resources.Media.MediaManager.GetMedia((MediaItem)imageItem);
                                                if (imageMedia != null)
                                                {
                                                    var extension = _filename.Split('.')[1];
                                                   
                                                    try
                                                    {
                                                        using (var output = new FileStream(_filename, System.IO.FileMode.Open))
                                                        {
                                                            imageMedia.SetStream(output, extension);
                                                        }
                                                    }
                                                    catch (Exception ex)
                                                    {
                                    ...
                                    //var lastImageName = System.IO.Path.GetFileNameWithoutExtension(entry.FilenameInZip);

                                    //var imageItem = parentMediaFolderItem.Children.Where(i => i.Name == lastImageName).FirstOrDefault();
                                    //if (imageItem != null)
                                    //{
                                    //    Sitecore.Resources.Media.Media imageMedia = Sitecore.Resources.Media.MediaManager.GetMedia((MediaItem)imageItem);
                                    //    if (imageMedia != null)
                                    //    {
                                    //        // imageMedia.SetStream(entry., extension);
            ...
            {
                sb.Append("Please select a zip file:").Append("<br/>");
            }
            count++;
...
<body>
    <form id="form1" runat="server">
        ...
        <table class="" style="width: 70%">
            ...
            <tr>
                <td colspan="2">
                    <asp:Button ID="btnPublish" runat="server" Text="Upload Media" OnClick="btnPublish_Click" /></td>
            </tr>
            <tr>
                <td colspan="2">
